  ...
  const submitGuess = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/words/english/validate?word=${encodeURIComponent(currentGuess)}`,
      );
      if (res.ok) {
      ...
      setTimeout(() => {
        ...
        if (token)
          fetch('/api/game-records', {
            ...
            body: JSON.stringify({ game: 'english-wordle', stars, score: currentRow + 1 }),
          })
            .then((r) => r.json())
      ...
      setTimeout(() => {
        ...
        if (token)
          fetch('/api/game-records', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
            body: JSON.stringify({ game: 'english-wordle', stars: 0, score: MAX_ATTEMPTS }),
          })
            .then((r) => r.json())
            .then((d) => { updateUser({ red_stars: d.red_stars, blue_stars: d.blue_stars, green_stars: d.green_stars }); saveProgress(); })
            .catch(() => {});
        ...
        {gameStatus !== 'playing' && (
          <div className='game-over-panel'>
            {gameStatus === 'won' ? (
              ...
              <>
                <div className='game-over-title'>
                  {Emojis.Sad} {t('Try_Again_Title_EN')}
                </div>
                <p className='game-over-text'>
                  {t('The_word_was_Prefix_EN')}{' '}
            ...
            <div className='game-over-buttons'>
              <button className='new-game-btn' onClick={() => startNewGame()}>
                {t('New_Game_EN')} {Emojis.Refresh}
              </button>
              <button
                className='new-game-btn secondary'
                onClick={() => navigate('/')}
              >
                {Emojis.House} {t('Back_to_Home_EN')}
              </button>
            </div>
          </div>
        )}
